...
namespace SARSCOV2.Controllers
{
    public class TableWojLastDayController : Controller
    {
        ...
        // public ActionResult Raport(/*string stan_rekordu_na*/)
           DataSet ds = new DataSet();
            string constr = ConfigurationManager.ConnectionStrings["C2"].ConnectionString;

            using (SqlConnection conn = new SqlConnection(constr))
            {
                string query = "SELECT stan_rekordu_na,b,c,d,e,f,g,h,i,j,k,l FROM PolandMapLastDayView";
                // WHERE stan_rekordu_na=@stan_rekordu_na";
